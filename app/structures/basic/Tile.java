	...
	public class Tile extends Observer {
		...
		enum TileState {
			NORMAL("normal", 0), WHITE("white", 1), RED("red", 2), LOCK_NORMAL("lock_normal", 0);
					...
					if (Integer.parseInt(String.valueOf(parameters.get("tilex"))) == this.tilex
						...
						// 这里会有两种情况： 1、目标对象在可直接攻击范围内 2、目标对象在可直接攻击范围外（攻击范围通过方法distanceOfTiles 判断）
							...
							if (distanceOfTiles(GameState.getInstance().getTileSelected(), this) <= 2) {
								...
								resetGameState();
								// only attack
								this.attackedBroadcast(GameState.getInstance().getTileSelected());
								}
							else {
...
//						}
						...
						else if(this.tileState == tileState.NORMAL){this.resetGameState();}
					}
				}
					...
					if (Integer.parseInt(parameters.get("tilex").toString()) == this.tilex
						...
						if (this.unitOnTile == null && this.tileState.equals(tileState.NORMAL)) {
				...
				else if (parameters.get("type").equals("searchUnit"))
				{
		...
		public void attack (Unit attacker, Unit beattacked){
			...
			newParameters.put("unit", beattacked);
			newParameters.put("attacker", attacker);
		...
		public void attackedBroadcast(Tile tileOfAttacker) {
			Map<String, Object>newParameters = new HashMap<>();
			newParameters.put("attackedUnit", this.getUnitOnTile());
			newParameters.put("attackedTile", this);
			newParameters.put("attackerUnit", tileOfAttacker.getUnitOnTile());
			newParameters.put("type", "attacked");
			//set source for this broadcast.
			if(this.isInAttackRange(tileOfAttacker)) {
				newParameters.put("counterAttack",1);
			} else newParameters.put("counterAttacker", 0);
